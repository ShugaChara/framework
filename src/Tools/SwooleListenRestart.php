 ...
 * Swoole 监听重启
 *
 *      $SwooleListenRestart = new SwooleListenRestart(app()->getBasePath(), 'php bin/cz http restart');
 * $SwooleListenRestart->listen();
 *
 *      注意：必须安装了inotify才能使用,不支持mac, pecl install inotify
 */
class SwooleListenRestart
{
    //监听的事件
    ...
    private function checkDepends()
    {
        if (! extension_loaded('inotify')) {
            throw new Exception('php inotify extension is required');
        }

        if (! extension_loaded('swoole')) {
            throw new Exception('php swoole extension is required');
        }
    }
